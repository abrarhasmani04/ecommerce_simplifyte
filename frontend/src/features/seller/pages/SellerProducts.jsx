...
const SellerProducts = () => {
  ...
  const fetchProducts = async (page) => {
    try {
      ...
      const list = data.products ?? data.data ?? data.result ?? data.items ?? [];
      const total = data.total ?? data.totalProducts ?? data.count ?? list.length;
      const tp =
        data.totalPages ?? data.pages ?? Math.max(1, Math.ceil(total / PAGE_SIZE));
      setProducts(list);
